...
class GridImageDataset(Dataset):
    ...
    def __init__(self, data_path, json_path, img_size, patch_size,
                 ...
                 sample_class_num={}, valid_number_ratio=0.5, is_test=False, stain_normalizer=False, cfg=None, total_tifs=None):
        """
    ...
    def _preprocess(self):
        ...
        if self._way == "valid":
                ...
                for file in os.listdir(valid_pth):
                    if self.is_test: # use all test dataset to test
                        sample_ratio = 1
                    else: # use a little test dataset as the valid dataset to adjust hyper-params
                        sample_ratio = self.valid_number_ratio
                    ...
                    try:
                        f = open(os.path.join(f_pth, 'list_uniq.txt'))
                    except:
                        f = open(os.path.join(f_pth, 'list.txt'))

                    lines = f.readlines()
                    line_index = random.sample(range(len(lines)), int(len(lines)*sample_ratio))
                    print("valid class number per epoch: ", file, len(line_index))
                    if len(lines) == 0 or len(lines[0].strip('\n').split(',')) == 4:
                        for idx in line_index:
...
#         dataset_train = GridImageDataset("/home/data2/medicine/20221031_69tif/patch_cls",
#                                         "/home/data2/medicine/20221031_69tif/json",
#                                         768,
#                                         256,
#                                         224,
#                                         sample_class_num={"cancer": 500,"cancer_beside": 126,"normal": 126,"hn": 126,"tl": 126,"other": 31},
