...
def make_tissue_mask(data_root_dir, tiff_path, save_location, key_word="HE_filepath"):
    os.makedirs(save_location, exist_ok=True)
    t1 = time.time()
    # print('start make_tissue_mask')
    opt_list = []
    for cur_name, file_info in tiff_path.items():
        file_path = os.path.join(data_root_dir, file_info[key_word])
