...
def parse_args():
    ...
    parser.add_argument('--parent_dir', type=str,
                        default="/media/server/Newsmy/WSI_PatchData/CY_SL_FD_HS/250715_res34_upload")
    # parser.add_argument('--patch_embedding_dir', type=str,
    #                     default="CONCH_Coord")
...
def load_patch_coord_from_folder(img_filefolder, already_done, embedding_filepath):
    samples_list = os.listdir(img_filefolder)
    process_list = []
    for sample_id in samples_list:
        embed_sample_dir = os.path.join(embedding_filepath, sample_id)
        ...
        for ipatch in os.listdir(os.path.join(img_filefolder, sample_id)):
            ...
            process_list.append({
                ...
                "patch_embedding_filepath": os.path.join(embedding_filepath, sample_id, f"{embed_name}.npz"),
            })
    return process_list
...
class Conch_MIL_fc(nn.Module):
    def __init__(self, size_arg="small", dropout=0., n_classes=2, ckpt_path="",
        ...
        self.conch, self.preprocess = create_model_from_pretrained(
            "conch_ViT-B-16", checkpoint_path="./CONCH/pretrain/pytorch_model.bin",
...
if __name__ == '__main__':
    ...
    # for dataset in ['huashan2', 'huashan', 'YouAn', 'hz', 'CY']:
    for dataset in [dataset_name]:
        # for dataset in ['huashan2']:
        print("Loading already done data")
        patch_embedding_dir = os.path.join(args.parent_dir, "patchEmbedding", dataset)
        os.makedirs(patch_embedding_dir, exist_ok=True)
        already_done = get_done_list(patch_embedding_dir)

        print("Loading undo data")
        img_filefolder = os.path.join(parent_dir, "patchImage", dataset)
        all_undo_img_list = load_patch_coord_from_folder(img_filefolder, already_done, patch_embedding_dir)
